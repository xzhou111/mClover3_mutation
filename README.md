...
```python
    ...
    #序列名称重命名
      awk 'BEGIN{n=0; keep=0} /^>/{if($0 ~ /^>mClover3/){keep=0; next}else{n++; print ">sample"n; keep=1; next}} keep{print}'       3.candidates/01_proteinmpnn/seqs/mClover3.fa > 3.candidates/01_proteinmpnn/mClover3.fa
    #对ProteinMPNN生成的 mClover3 候选序列进行 ESM-2 打分，然后按分数从高到低排序，筛出 Top 候选序列
